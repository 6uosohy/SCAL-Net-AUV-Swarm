# Fig. 12 — total and average energy consumption by role group (CH vs CM) in
# search mode, over cluster configurations N = 1..5.
import os
import matplotlib
matplotlib.use("Agg")
...
total_ch, total_cm = [], []
for n in N_VALUES:
    ch, cm = energy_by_role(os.path.join(DATA, "search_mode", f"energy_N{n}.json"))
    total_ch.append(sum(ch))
    # One CM per cluster, which is the convention the published figure uses.
    # See docs/figure_data_map.md — the run carries two CMs per cluster.
...
fig.tight_layout()
save(fig, "fig12a_search_total_energy")
plt.close(fig)
...
bars(ax, avg_ch, avg_cm,
     ...
     "$E_{avg,CH}$", "$E_{avg,CM}$")
fig.tight_layout()
save(fig, "fig12b_search_average_energy")
plt.close(fig)
